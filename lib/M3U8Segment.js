...
var NOTIFY_OTHER            = 0; // その他
var NOTIFY_CACHED_DURATION  = 1; // cachedDuation が更新された
var NOTIFY_TS_CACHED        = 2; // ts をキャッシュした
var NOTIFY_CACHE_LIMIT      = 3; // cache limit をオーバーした
var NOTIFY_CONNECTION_LIMIT = 4; // connection limit をオーバーした
// --- class / interfaces ----------------------------------
function M3U8Segment(playlist,  // @arg MediaPlaylistObject - { url, type, version, allowCache, mediaSequence, mediaSegments, targetDuration, totalDurations }
                     options) { // @arg Object - { updateCallback, errorCallback, m3u8Callback, tsCallback, endCallback, m3u8FetchIntervalRatio }
    this._m3u8URL           = playlist["url"];
    this._updateCallback    = options["updateCallback"]; // updateCallback(cachedDurations:UINT32, notify:UINT8):void
    ...
    this._live              = /LIVE/.test(playlist["type"]); // LIVE or NRTLIVE
...
M3U8Segment["NOTIFY_OTHER"]            = NOTIFY_OTHER;
M3U8Segment["NOTIFY_CACHED_DURATION"]  = NOTIFY_CACHED_DURATION;
M3U8Segment["NOTIFY_TS_CACHED"]        = NOTIFY_TS_CACHED;
M3U8Segment["NOTIFY_CACHE_LIMIT"]      = NOTIFY_CACHE_LIMIT;
M3U8Segment["NOTIFY_CONNECTION_LIMIT"] = NOTIFY_CONNECTION_LIMIT;
...
function _dequeue() {
    var stateMap = _countState.call(this); // { STATE_NEW, STATE_LOADING, STATE_CACHED, STATE_RESERVED, STATE_USED, STATE_ERROR }

    if (this._suppressMemoryUsage) {
        if (stateMap[STATE_CACHED]   >= this._maxBlobCached ||
            stateMap[STATE_RESERVED] >= this._maxBlobReserved) {
            // キャッシュ残量が既定値を超えている場合はupdateCallbackを呼んで処理を中断する
            this._updateCallback(this._cachedDurations, NOTIFY_CACHE_LIMIT);
            return;
        }
    }

    for (var i = this._view.cursor, iz = this._view.tsIDs.length; i < iz; ++i) {
        if (this._suppressConnectionUsage &&
            this._connections >= this._maxConnections) {
            // コネクション数が既定値を超えている場合はupdateCallbackを呼んで処理を中断する
            this._updateCallback(this._cachedDurations, NOTIFY_CONNECTION_LIMIT);
            break;
        }
        ...
        if (tsInfo["state"] === STATE_NEW) {
            ...
            _loadTSFile(this, tsID, tsInfo["tsURL"]);
        }
    }
    if (stateMap[STATE_CACHED] > 0) {
        _updateCachedDurations.call(this);
        this._updateCallback(this._cachedDurations, NOTIFY_CACHED_DURATION);
    }
}

function _loadTSFile(that, tsID, tsURL) {
    ...
    FileLoader["loadBlob"](tsURL, function(tsBlob) {
        ...
        var tsInfo = that._tsInfoMap[tsID];
        // 非同期動作なので、タイミングによっては tsInfoMap がクリアされている場合がある
        // tsInfo の存在と状態を確認した上で処理を進める
        if (tsInfo && tsInfo["state"] === STATE_LOADING) { // STATE_LOADING -> STATE_CACHED
            ...
            that._tsCallback(tsID, tsInfo["tsURL"], tsBlob);
            that._updateCallback(that._cachedDurations, NOTIFY_TS_CACHED);
        }
    ...
    for (var i = 0, iz = this._view.tsIDs.length; i < iz; ++i) { // [tsID, ...]
        ...
        if (seekTime >= startTime && seekTime <= endTime) { // include seek time
          //var a = this._view.cursor; // point a, old head
            ...
            _dequeue.call(this); // seek直後でキャッシュが空なため素早く再取得を行う
